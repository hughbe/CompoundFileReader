...
internal struct CompoundFileHeader: CustomDebugStringConvertible {
    ...
    internal init(data: inout DataStream) throws {
        ...
        self.signature = try data.read(endianess: .littleEndian)
        ...
        // (CLSID_NULL).
        self.clsid = try data.read(type: GUID.self)
        ...
        self.minorVersion = try data.read(endianess: .littleEndian)
        ...
        self.majorVersion = try data.read(endianess: .littleEndian)
        ...
        self.byteOrder = try data.read(endianess: .littleEndian)
        ...
        self.sectorShift = try data.read(endianess: .littleEndian)
        ...
        self.miniSectorShift = try data.read(endianess: .littleEndian)
        ...
        self.numberOfDirectorySectors = try data.read(endianess: .littleEndian)
        
        // Number of FAT Sectors (4 bytes): This integer field contains the count of the number of FAT
        // sectors in the compound file.
        self.numberOfFATSectors = try data.read(endianess: .littleEndian)
        
        // This integer field contains the starting sector number for
        // the directory stream
        self.firstDirectorySectorLocation = try data.read(endianess: .littleEndian)
        ...
        // implemented.
        self.transactionSignatureNumber = try data.read(endianess: .littleEndian)
        ...
        self.miniStreamCutoffSize = try data.read(endianess: .littleEndian)
        
        // First Mini FAT Sector Location (4 bytes): This integer field contains the starting sector number for
        // the mini FAT.
        self.firstMiniFATSectorLocation = try data.read(endianess: .littleEndian)
        ...
        self.numberOfMiniFATSectors = try data.read(endianess: .littleEndian)
        
        // First DIFAT Sector Location (4 bytes): This integer field contains the starting sector number for
        // the DIFAT
        self.firstDIFATSectorLocation = try data.read(endianess: .littleEndian)
        
        // Number of DIFAT Sectors (4 bytes): This integer field contains the count of the number of DIFAT
        // sectors in the compound file
        self.numberOfDIFATSectors = try data.read(endianess: .littleEndian)
